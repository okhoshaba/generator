...
//      long srt;
      ...
      for (int countPeriod = 0; countPeriod < Config.signalOfSamplingRate; countPeriod++) {
 //       srt = 0L;
      // for diagnostic only
//    	  process = runtime.exec("ping 192.168.1.1 > /dev/null 2>&1");
//    	for Amplitude Generation  
//     	  for (int dots = 0; dots < 10; dots++) {
    	  for (int count = 0; count < Config.amplitude[Config.countAmplitude]; count++) {
//    	  for (int countAmplitude = 0; countAmplitude < 40; countAmplitude++) {
...
// for diagnostic only
          System.out.println("lev0 ; " + "executionOrder ; " + this.cycle + " ; countPeriod ; " + countPeriod + " ; countAmplitude ; " + count + " ; rt ; " + processDuration);
//          srt += processDuration;
        }  // end of countAmplitude

//        mrt = 10 * Math.log10((double)srt/(double)Config.periodOfTime);
//        System.out.println("lev1 ; " + " Amplitude ; " + Config.amplitude[Config.countAmplitude] + " ; executionOrder ; " + this.cycle + " ; countPeriod ; " + (countPeriod+1) + "; srt ; " + srt + " ; mRT ; " + mrt);
        Config.countAmplitude++;
          if (Config.countAmplitude > 9)
            Config.countAmplitude = 0;
//        }  // end for dots
      }  // end for countPeriod
      
      // for diagnostic only
        long  totalStop = System.currentTimeMillis();
        long  totalProcessDuration = totalStop - totalStart;
        System.out.println("lev2 ; " + "totalProcessDuration ; " + totalProcessDuration);
